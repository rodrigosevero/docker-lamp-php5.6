...
class FuncionalidadeTable
{
    ...
    {
        $sql = 'select a.* from funcionalidade a where a.funcionalidade_pai is NULL and funcionalidade_id > 2';
        $statement = $this->tableGateway->adapter->query($sql);
        return $statement->execute();
    }


    public function getFuncionalidadeFilho($id)
    {
        $sql = 'select a.* from funcionalidade a where a.funcionalidade_pai = '.$id;
        $statement = $this->tableGateway->adapter->query($sql);
        return $statement->execute();
    }
